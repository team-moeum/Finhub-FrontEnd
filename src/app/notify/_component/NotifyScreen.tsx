        ...
        >
          ...
          {!receivedAt && (
            <Box
              ...
              height={10}
              radius="50%"
              backgroundColor="#FE835C"
            />
          )}
        </FlexBox>
        <Stack gap={3}>
          <Text size={15} weight={600} color="#25292C" style={getTextOverflowStyle(1)}>
            {title}
          </Text>
          <Text size={12} weight={500} color="#A6ABAF" style={getTextOverflowStyle(3)}>
            {message}
          </Text>
          <Text size={10} weight={400} color="#CDD1D5">
            {parseDate(sentAt)}
          </Text>
        </Stack>
      </FlexBox>
...
export const NotifyScreen = () => {
  ...
  const { data, hasNextPage, fetchNextPage } = useAlarmInfiniteQuery({ size: 10 });

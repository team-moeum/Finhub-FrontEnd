...
export const EditCommentSheet = ({ show,columnId, onClose, onDeleteClick }: EditCommentSheetProps) => {
  return (
      ...
      <Stack gap={10} px={16} py={32}>
        <LinkBox href={`/feed/column/${columnId}/comment/post?type=${COMMENT_POST_TYPE.edit}`}>
          <FlexBox justifyContent='flex-start' gap={15} py={10}>
            <EditIcon />
            <Text size={24} weight={500} color="#494F54">수정하기</Text>
          </FlexBox>
        </LinkBox>
        <PressBox onClick={onDeleteClick}>
          <FlexBox justifyContent='flex-start' gap={15} py={10}>
            <DeleteIcon />
            <Text size={24} weight={500} color="#494F54">삭제하기</Text>
          </FlexBox>
        </PressBox>
      </Stack>
    </BottomSheet>

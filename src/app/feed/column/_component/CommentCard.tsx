...
import {
  COLUMN_LIKE_TYPE,
  useBanGptColumnComment,
  useDeleteGptColumnComment,
  useGptColumnLike
...
export const CommentCard = ({ id, isMine, comment, refetch }: CommentCardProps) => {
  ...
  return (
    <>
      <Box px={16} py={12} radius={20} backgroundColor="#F6F7F9">
        <Stack gap={14}>
          ...
          <FlexBox justifyContent="flex-start" gap={4}>
            ...
            <Box onClick={handleCommentClick}>{like ? <LikeIconGreenOn /> : <LikeIconGray />}</Box>
          </FlexBox>
        </Stack>
      </Box>
      ...
      />
      <BanCommentSheet
        show={BanCommentSheetModal.show}
        columnId={comment.id}
        commentId={comment.id}
        onClose={BanCommentSheetModal.close}
        onBanClick={handleBanPopupOpen}
      />
      <DeleteCommentPopup
      ...
      />
      <BanCommentPopup
        show={BanCommentPopupModal.show}
        name={comment.nickname}
        onCancel={BanCommentPopupModal.close}
        onBan={handleBanUser}
      />

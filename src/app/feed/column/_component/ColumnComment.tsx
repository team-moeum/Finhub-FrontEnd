...
export const ColumnComment = ({ columnId, pageType }: ColumnCommentProps) => {
  ...
  const { data, refetch, hasNextPage, fetchNextPage, isLoading } = useGptColumnCommentList({
    id: columnId,
    type: TAB_NUMBER[tab],
    size: pageType === PAGE_TYPE.columnDetail ? 3 : 7
  });
  const commentList = useMemo(
    () => (data ? data.pages.flatMap(data => data.comments) : []),
    [data]
  );
      ...
      <Container>
        ...
        <Box mt={30} height={100} radius={20} backgroundColor="#EDF0F3">
          <FlexBox gap={16} height="100%">
            <SpeechBubbleIcon />
            <Text size={14} weight={600} color="#A6ABAF">
              아직 나눈 의견이 없어요. 의견을 남겨주세요!
            </Text>
          </FlexBox>
        </Box>
      </Container>
      ...
      <FlexRow>
        ...
        <FlexBox gap={12}>
          <Box onClick={handlePopularTabClick}>
            <Text size={14} weight={600} color={tab === "popular" ? "#191B1C" : "#CDD1D5"}>
              인기순
            </Text>
          </Box>
          <Box onClick={handleRecentTabClick}>
            <Text size={14} weight={600} color={tab === "recent" ? "#191B1C" : "#CDD1D5"}>
              최신순
            </Text>
          </Box>
        </FlexBox>
            ...
            {commentList?.map(item => {
              return (
                <CommentCard
                  ...
                  id={item.id}
                  comment={item}
                  isMine={item.nickname === userInfo?.nickname}
          ...
          {pageType === PAGE_TYPE.columnDetail && commentList?.length !== 0 && (
            <LinkButton href={`/feed/column/${columnId}/comment`}>
              <FlexBox height={36} backgroundColor="#F6F7F9" radius={10}>
                <Text size={15} weight={600} color="#A6ABAF">
                  더보기
                </Text>
              </FlexBox>
            </LinkButton>

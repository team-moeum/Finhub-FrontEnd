  ...
  return (
    <Link href={`/${item.categoryId}/${item.topicId}`}>
      <div className={style.item_box}>
        <span className={style.title}>{highlightKeyword(keyword, item.title)}</span>
        <span className={style.content}>{highlightKeyword(keyword, item.summary)}</span>
      </div>
    </Link>
  );
};
  ...
  return (
    <>
      <div className={style.container}>
        {resultSearchList &&
          resultSearchList.map((item, index) => <Item key={index} keyword={keyword} item={item} />)}
      </div>
